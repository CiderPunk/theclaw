...
use crate::{asset_loader::SceneAssets, collision_detection::{Collider, Player}, health::Health, hook::{Hook, HookReturnedEvent, HOOK_DAMPING, HOOK_LAUNCH_SPEED, HOOK_MAX_SPEED}, movement::{Acceleration, Velocity}, scheduling::GameSchedule, state::GameState};
...
impl Plugin for ShipPlugin{
  fn build(&self, app: &mut App){
    app
      ...
      .add_systems(Update, (movement_controls, update_pitch, fire_controls).chain().in_set(GameSchedule::UserInput))
...
fn fire_controls(
  ...
  let Ok((entity, mut ship, velocity)) = query.get_single_mut()
  else{
    return;
  };
  if keyboard_input.pressed(KeyCode::Space) && !ship.hook_out{
    ...
    *display_hook_visible = Visibility::Hidden;
    ...
    commands.spawn((
      ...
      Acceleration{ acceleration:Vec3::ZERO, damping:HOOK_DAMPING, max_speed:HOOK_MAX_SPEED,},
...
fn retrieve_hook(
  ...
  for &HookReturnedEvent{ target } in ev_hook_returned.read(){
    let Ok(mut visible) = display_hook_query.get_single_mut() 
    else{ return; };
    
    *visible = Visibility::Visible;

    let Ok(mut ship) = ship_query.get_single_mut() 
    else{ return; };
    ship.hook_out = false;
