...
use crate::{constants::GRAVITY, movement::{Acceleration, Roller, Velocity}, scheduling::GameSchedule, splosion::SplosionEvent};
...
impl WreckedEvent{
  pub fn new(scene:Handle<Scene>, translation:Vec3, quat:Quat, velocity:Vec3, roll_speed:f32, time_to_live: f32, blast_size:f32) -> Self {
    Self { scene, translation, quat, velocity, roll_speed, time_to_live, blast_size }
  }
}
...
fn update_wrecks(
  ...
  for (entity, mut wreck, transform, velocity) in query.iter_mut(){
    ...
    if wreck.time_to_blast.just_finished(){
      ev_splosion_writer.send(SplosionEvent::new(transform.translation(), wreck.blast_size * wreck.time_to_live.fraction() ,velocity.0));
    }    
...
fn spawn_wrecks(
  ...
  for WreckedEvent{ scene, translation, quat, velocity, roll_speed, time_to_live, blast_size } in ev_wrecked_reader.read(){
    ...
    ev_splosion_writer.send(SplosionEvent::new(*translation + Vec3::new(0.,0.,-2.0), *blast_size, *velocity));
