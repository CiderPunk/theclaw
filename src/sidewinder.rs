...
fn check_dead(
  ...
  for(entity, health, transform, velocity) in query.iter(){
   ...
   //   ev_splosion_writer.send(SplosionEvent::new(transform.translation(), 3.0,velocity.0));
      ev_wreck_writer.send(WreckedEvent::new(scene_assets.sidewinder.clone(), transform.translation(), transform.rotation(),  velocity.0, SIDEWINDER_SPIN_SPEED, 2.0, SIDEWINDER_BLAST_SIZE));
