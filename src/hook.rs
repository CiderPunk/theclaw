...
pub enum HookControlEventType {
  Launch,
  Return,
}

#[derive(Event)]
pub struct HookControlEvent {
  owner: Entity,
  event_type: HookControlEventType,
  location: Vec3,
  base_velocity: Vec3,
}

impl HookControlEvent {
  pub fn new(
    owner: Entity,
    event_type: HookControlEventType,
    location: Vec3,
...
fn hook_launch(
  ...
  mut ev_hook_launch: EventReader<HookControlEvent>,
  scene_assets: Res<SceneAssets>,
) {
  for HookControlEvent {
    owner,
    event_type,
    location,
    base_velocity,
...
fn apply_collisions(
  ...
  mut target_query: Query<
    (&mut Transform, &mut Velocity, &GlobalTransform, &Hookable),
    Without<Hook>,
  >,
) {
  for &CollisionEvent { entity, collided } in ev_collision.read() {
    ...
    let Ok((mut transform, mut velocity, target_transform, hookable)) =
    ...
    commands.entity(collided).insert(Hooked {
      time: Stopwatch::new(),
    });
  }
}
