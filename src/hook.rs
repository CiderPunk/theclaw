...
use crate::{collision_detection::{CollisionEvent, Player}, movement::{Acceleration, Velocity}, scheduling::GameSchedule};
...
fn update_hook(
  ...
  let Ok((mut hook, hook_transform, mut acceleration)) = query.get_single_mut() 
  else{ return; };

  let Ok(owner_transform) = owner_query.get(hook.owner)
  else{ return; };
